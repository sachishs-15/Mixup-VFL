...
class ClientModel(nn.Module):
    def __init__(self, input_size: int, hidden_layers: List[int], embedding_size: int):
        super().__init__()
        # Replace neural network with single linear layer
        self.layers = nn.Linear(input_size, embedding_size)
...
class ServerModel(nn.Module):
    def __init__(self, num_clients: int, embedding_size: int, hidden_layers: List[int]):
        ...
        self.layers = nn.Linear(total_input_size, 1)
...
class CustomizableVFL:
    ...
    def train_step(self, client_batch_data: List[torch.Tensor], client_batch_labels: List[torch.Tensor]) -> float:
        """Modified training step with top model coordination and ridge regression"""
        # Reset gradients
        ...
        # Calculate MSE loss
        mse_loss = self.loss_fn(final_prediction, self.mixup_fn(client_batch_labels, client_embeddings))
    
        # Add L2 regularization term for ridge regression
        l2_reg = 0.0
...
def fetch_data_superconductivity():
    ...
    df = df = df.applymap(lambda x: str(x).replace(",", ".") if isinstance(x, str) else x)
    df = df.astype(float)
...
def run_program():
    ...
    # Load data
    X, y, feat_no = fetch_data_superconductivity()
    # set_trace()
    print("data loaded")
    
    def create_feature_splits(total_features, num_clients=3):
        split_size = total_features // num_clients  # Divide features equally
        remainder = total_features % num_clients  # Handle any leftover features
        feature_splits = []
        start = 0

        for i in range(num_clients):
            end = start + split_size + (1 if i < remainder else 0)  # Distribute remainder
    ...
    # Configuration
    num_clients = 3
    feature_splits = create_feature_splits(feat_no)
    ...
    print(f"Final Best RMSE: {results['best_rmse']}")
